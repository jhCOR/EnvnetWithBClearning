...
class Trainer:
    ...
    def train(self, epoch):
        ...
        for i, batch in enumerate(self.train_iter):
            ...
            x = torch.Tensor(x_array[:, None, None, :], device="cuda")
            t = torch.Tensor(t_array, device="cuda")
            ...
            if self.opt.BC:
                ...
                acc = accuracy(y, torch.argmax(t, axis=1))
            else:
                loss_fun = nn.CrossEntropyLoss()
                loss = loss_fun(y, t.softmax(dim=1))
                acc = accuracy(y, t)
    ...
    def val(self):
        ...
        for batch in self.val_iter:
            ...
            y = nn.Softmax(self.model(x))
            y = torch.reshape(y, (y.shape[0] // self.opt.nCrops, self.opt.nCrops, y.shape[1]))
