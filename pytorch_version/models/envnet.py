...
class EnvNet(nn.Module):
    ...
    def __call__(self, x):
        ...
        h = nn.functional.max_pool2d(h, (1, 160))
        h = torch.swapaxes(h, 1, 2)

        h = self.conv3(h, self.train)
        h = nn.functional.max_pool2d(h, 3)
        h = self.conv4(h, self.train)
        h = nn.functional.max_pool2d(h, (1, 3))

        if self.train is True:
            h = nn.Dropout(nn.ReLU(self.fc5(h)))
            h = nn.Dropout(nn.ReLU(self.fc6(h)))
